$(document).ready(function() {
    ...
    test('init', 4, function() {
        ...
        equal($('#' + TEST_CONTAINER_ID).children('.rdfauthor-predicate-row').length, 1, 'Test container should have 1 child of class property-row.');
        equal($('#' + TEST_CONTAINER_ID).children('.rdfauthor-predicate-row').attr('id'), this.fixture._idPrefix + 123, 'Property row id should be ' + this.fixture._idPrefix + 123);
    ...
    test('addWidget', 4, function() {
        ...
        equal($('#' + TEST_CONTAINER_ID).children('.rdfauthor-predicate-row').children('fieldset').children('.widget').length, 1, 'HTML for one widget should be in DOM.');
        equal($('#' + TEST_CONTAINER_ID).children('.rdfauthor-predicate-row').children('fieldset').children('#' + id).attr('id'), id, 'Widget CSS id should be the id returned.');
    });
